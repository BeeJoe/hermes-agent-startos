...
// configured. Run with the venv python so hermes_cli is importable.
// TODO(build): confirm the module path / function name against upstream.
...
export const main = sdk.setupMain(async ({ effects }) => {
  ...
  return sdk.Daemons.of(effects)
    ...
    .addDaemon('dashboard', {
      subcontainer: sub,
      exec: {
        // TODO(build): the dashboard validates the Host header against its bind
        // address; binding 0.0.0.0 may need an allowed-hosts flag (no Umbrel proxy here).
        command: [
          'hermes',
          'dashboard',
          '--host',
    ...
    .addDaemon('bundle-refresh', {
      subcontainer: sub,
      exec: {
        // TODO(build): confirm bundle URL + ETag handling; tune the interval.
        command: [
          'sh',
          '-c',
          `while true; do ` +
